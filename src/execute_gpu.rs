...
type my_dtype_u = u32;
use crate::{
    decoderiter::{self, FrameProvider},
    error::{Error, Result},
    gpu_setup::{self, GpuState, GpuStateFlavor, ParamStyle, TrackingParams},
    into_slice::IntoSlice,
    kernels,
    linking::{
...
fn submit_work<'a, F: IntoSlice>(
    ...
    state
        .queue
        .write_buffer(staging_buffer, 0, bytemuck::cast_slice(frame_slice));
    ...
    encoder.copy_buffer_to_buffer(
        ...
        0,
        common_buffers.counter_buffer.size(),
    );
    ...
    if tracking_params.illumination_sigma.is_some() {
        if tracking_params.illumination_correction_per_frame {
            ...
            common_buffers
                ...
                .unwrap()
                .pass
                .execute(&mut encoder, bytemuck::cast_slice(&[sigma, 1.0]));
        ...
        // }
        state.passes["correct_illumination"][0]
            .execute(&mut encoder, bytemuck::cast_slice(&[state.pic_size as u32]));
    }
    ...
    match state.flavor {
        GpuStateFlavor::Trackpy { ref order, .. } => {
            if let Some(SubsetterOutput::Characterization(ref positions)) = positions {
                ...
                state.queue.write_buffer(
                    &common_buffers.atomic_filtered_buffer,
                    0,
                    bytemuck::cast_slice(&[positions.len() as u32]),
                );
                encoder.copy_buffer_to_buffer(
                    staging_buffer,
                    state.pic_byte_size,
                    &common_buffers.result_buffer,
                    0,
                    state.pic_byte_size,
                );
                let (mut charpass, workgroup1d) = state
                    .characterize
                    .take()
                    .expect("The characterize pass should always be set at this point");
                charpass.dispatcher = Some(Rc::new(Dispatcher::new_direct(
                    &[positions.len() as u32, 1, 1],
                    &workgroup1d,
                )));
    ...
    if let Some(n_iter) = tracking_params.adaptive_background {
        ...
        let mut iter = the_atomics
            .iter()
            .zip(state.passes["adaptive_filter"].iter())
            .cycle()
            .take(n_iter);
        state.common_buffers.raw_frame.as_ref().map(|buf| {
            encoder.copy_buffer_to_buffer(&state.common_buffers.frame_buffer, 0, buf, 0, buf.size())
        });
    ...
    encoder.copy_buffer_to_buffer(output_buffer, 0, staging_buffer, 4, state.pic_byte_size);
...
fn get_work<'a>(
    ...
    state
        .device
        .poll(wgpu::Maintain::WaitForSubmissionIndex(old_submission));
    wait_gpu_time
        .as_mut()
        .map(|t| *t += now.unwrap().elapsed().as_secs_f64());
    ...
    let particle_data_endpoint =
        (n_parts as usize) * MY_DTYPE_SIZE * N_RESULT_COLUMNS + MY_DTYPE_SIZE;
    let results =
        bytemuck::cast_slice::<u8, ResultRow>(&data[MY_DTYPE_SIZE..particle_data_endpoint])
            .to_vec();
...
fn handle_thread_error<'a, E, T>(
    send_res: std::result::Result<(), E>,
    thread_handle: ScopedJoinHandle<'a, Result<T>>,
) -> Result<ScopedJoinHandle<'a, Result<T>>> {
    match send_res {
...
where
    F1: Fn(my_dtype) -> (Vec<[i32; 2]>, usize),
    F2: Fn(my_dtype) -> Vec<[i32; 2]>,
{
...
fn post_process<A: IntoSlice>(
    ...
    kernels: Option<
        &mut PostProcessKernels<
            impl Fn(my_dtype) -> (Vec<[i32; 2]>, usize),
            impl Fn(my_dtype) -> Vec<[i32; 2]>,
        >,
    >,
    frame: Option<A>,
    dims: [u32; 2],
    ...
    let raw_properties = frame.map(|inner| {
        ...
        for row in relevant_points.iter() {
            ...
            for ind in raw_sig_inds {
                let curidx = [
                    (middle_index[0] + ind[0]) as usize,
                    (middle_index[1] + ind[1]) as usize,
                ];
            ...
            let bg_median = if bg.len() == 0 { 0. } else { bg[bg.len() / 2] };
    ...
    if let (Some(linker), Some(reset_points)) =
        (&mut linker, tracking_params.linker_reset_points.as_ref())
    {
    ...
    for (idx, row) in relevant_points.iter().enumerate() {
        ...
        part_ids
            .as_ref()
            .map(|part_ids| output.push(part_ids[idx] as my_dtype));
    }
    Ok(())
}

pub fn mean_from_iter<A: IntoSlice, T: Iterator<Item = Result<A>>>(
    mut iter: T,
    dims: &[u32; 2],
) -> crate::error::Result<Array2<my_dtype>> {
    let mut n_frames = 0;
...
pub fn setup_illumination_profile(
    ...
    state.queue.write_buffer(
        staging_buffer,
        0,
        bytemuck::cast_slice(mean_frame.into_slice()),
    );
...
pub fn execute_gpu<F: IntoSlice + Send, P: FrameProvider<Frame = F> + ?Sized>(
    ...
    let (inp_sender, inp_receiver) = std::sync::mpsc::channel();
    ...
    let output = std::thread::scope(
        |scope: &Scope| -> Result<(Vec<f32>, Option<Vec<crate::linking::DurationBookkeep>>)> {
            let handle = {
                ...
                let mut linker = tracking_params
                    .search_range
                    .map(|range| Linker::new(range, tracking_params.memory.unwrap_or(0)));
                let progress = progress;
                scope.spawn(
                    move || -> Result<(Vec<f32>, Option<Vec<crate::linking::DurationBookkeep>>)> {
                        let mut kernels = if send_frame {
                            ...
                            let raw_bg_inds = FloatMemoizer::new(|radius| {
                                kernels::annulus_inds(
                                    tracking_params.bg_radius.unwrap_or(2.0 * radius),
                                    tracking_params.gap_radius.unwrap_or(0.) + radius,
                                )
                            });
                            Some(PostProcessKernels {
                                raw_sig_inds,
                                raw_bg_inds,
                            })
                        } else {
                        ...
                        loop {
                                ...
                                .map_err(|_| crate::error::Error::ThreadError)?
                            {
                                None => break,
                                Some(inp) => {
                                    thread_sleep.as_mut().map(|thread_sleep| {
                                        *thread_sleep +=
                                            now.unwrap().elapsed().as_nanos() as f64 / 1e9
                                    });
                                    let (results, frame_index, _neighborhoods) = inp;
                                    let frame = match frame_receiver {
                                        Some(ref receiver) => Some(
                                            receiver
                                                .recv()
                                                .map_err(|_| crate::error::Error::ThreadError)?,
                                        ),
                        ...
                        thread_sleep
                            .map(|thread_sleep| println!("Thread sleep: {} s", thread_sleep));
            ...
            let mut free_staging_buffers = state
                .common_buffers
                .staging_buffers
                .iter()
            ...
            let mut the_iter: Box<dyn Iterator<Item = crate::error::Result<_>>> = match pos_iter {
                Some(subsetter) => match subsetter.frame_col {
                    Some(_) => Box::new(subsetter.map(|result| {
                        let (frame_to_take, positions) = result?;
                        let frame_idx = frame_to_take.unwrap();
                        let frame = frames.get_frame(frame_idx).map_err(|err| match err {
                            Error::FrameOOB => Error::FrameOutOfBounds {
                                vid_len: frames.len(Some(frame_idx)),
                                problem_idx: frame_idx,
                            },
                            _ => err,
                        })?;
                        ...
                        Some(keys) => {
                            if keys.iter().enumerate().all(|(idx, &key)| idx == key) {
                                ...
                                Box::new(keys.into_iter().map(|&frame_idx| {
                                    let frame =
                                        frames.get_frame(frame_idx).map_err(|err| match err {
                                            Error::FrameOOB => Error::FrameOutOfBounds {
                                                vid_len: frames.len(Some(frame_idx)),
                                                problem_idx: frame_idx,
                                            },
            ...
            let (mut old_submission, mut handle) = submit_work(
                frame,
                staging_buffer,
                state,
                &tracking_params,
            ...
            for res in the_iter {
                ...
                let (new_submission, new_handle) = submit_work(
                    frame,
                    staging_buffer,
                    &state,
                    &tracking_params,
    ...
    let output = match (part_durations, particle_column) {
        (Some(durations), Some(particle_column)) => {
            ...
            for chunk in output.chunks_exact(col_names.len() - 1) {
                ...
                new_output.push(durations[chunk[particle_column] as usize].duration as f32)
            }
            new_output
        }
        (None, None) => output,
...
pub fn path_to_iter<P: AsRef<std::path::Path>>(
    path: P,
    channel: Option<usize>,
) -> crate::error::Result<(
    Box<
        dyn FrameProvider<Frame = Vec<f32>, FrameIter = Box<dyn Iterator<Item = Result<Vec<f32>>>>>
            + 'static,
    >,
    [u32; 2],
)> {
    ...
    let ext = path
        .extension()
        .ok_or_else(|| Error::NoExtensionError {
    ...
    let (iter, dims): (
        Box<
            dyn FrameProvider<
                Frame = Vec<my_dtype>,
                FrameIter = Box<dyn Iterator<Item = Result<Vec<f32>>>>,
            >,
        >,
        _,
    ) = match ext.to_str().unwrap() {
        "tif" | "tiff" => {
            let file = File::open(path).map_err(|ioerr| crate::error::Error::FileNotFound {
                source: ioerr,
                filename: path.to_path_buf(),
            })?;
        ...
        "ets" => {
            let mut file = File::open(path).map_err(|ioerr| crate::error::Error::FileNotFound {
                source: ioerr,
                filename: path.to_path_buf(),
            })?;
        ...
        "vsi" => {
            ...
            let mut file =
                File::open(ets_path).map_err(|ioerr| crate::error::Error::FileNotFound {
                    source: ioerr,
                    filename: path.to_path_buf(),
                })?;
            let parser = decoderiter::MinimalETSParser::new(&mut file).unwrap();
        ...
        _ => Err(crate::error::Error::UnsupportedFileformat {
...
>(
    ...
    let pos_iter = match pos_array {
        Some((pos_array, true, true)) => Some(FrameSubsetter::new(
            ...
            (1, 2),
            Some(3),
            SubsetterType::Characterization,
        )),

        Some((pos_array, true, false)) => Some(FrameSubsetter::new(
            pos_array,
            Some(0),
            (1, 2),
            None,
        ...
        Some((pos_array, false, true)) => Some(FrameSubsetter::new(
            pos_array,
            None,
            (0, 1),
            Some(2),
        ...
        Some((pos_array, false, false)) => Some(FrameSubsetter::new(
            pos_array,
            None,
            (0, 1),
            None,
    ...
    let (res, column_names) = execute_gpu(
        provider,
        dims,
        &params,
        verbosity,
    ...
    let res = Array2::from_shape_vec(shape, res).expect(
        format!(
            "Could not convert to ndarray. Shape is ({}, {}) but length is {}",
            shape.0, shape.1, &res_len
        )
        .as_str(),
    );
    Ok((res, column_names))
