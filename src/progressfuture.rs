...
where
    R: Send + 'static,
    P: Send + Default + Copy + 'static,
    J: Send + 's,
{
    pub fn from_interrupt_signal<'env: 's, F>(
        scope: &'s Scope<'s, 'env>,
        interrupter: Arc<AtomicBool>,
        function: F,
    ) -> Self
    where
        ...
        let handle = {
            ...
            scope.spawn(move || loop {
                match job_receiver.recv() {
                    Ok((job, interrupter)) => {
                        match result_sender.send(function(job, &progress, &interrupter)) {
                            Ok(()) => (),
                            Err(_) => break,
                        }
                    }
    ...
    pub fn submit_same(&mut self, job: J) -> Result<(), Error> {
        let res = self
            .job_sender
            .send((job, self.interrupter.clone()))
