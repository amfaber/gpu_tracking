...
use tracing::*;
use wgpu::{self, util::DeviceExt, BindGroupLayout, Buffer, ComputePipeline, Device};
...
impl CommonBuffers {
    fn create(
        ...
        pipelines: &mut HashMap<
            String,
            (Rc<ComputePipeline>, BindGroupLayout, Option<Rc<Dispatcher>>),
        >,
    ) -> Self {
        ...
        let raw_frame =
            if tracking_params.adaptive_background.is_some() & tracking_params.characterize {
...
impl TrackpyGpuBuffers {
    fn create(device: &wgpu::Device, size: u64) -> Self {
...
fn gpuparams_from_tracking_params(params: &TrackingParams, pic_dims: [u32; 2]) -> GpuParams {
    let (kernel_size, circle_size, dilation_size, margin, sigma) = match params.style {
        ParamStyle::Trackpy {
            ...
            let margin = vec![diameter / 2, separation / 2 - 1, smoothing_size / 2]
                .iter()
                .max()
                .unwrap()
                .clone() as i32;
    ...
    GpuParams {
        ...
        rough_snr_factor: if params.adaptive_background.is_some() {
            0.5
        } else {
            1.0
        },
...
pub fn new_adapter() -> wgpu::Adapter {
...
pub fn setup_state<'a>(
    tracking_params: &TrackingParams,
    dims: &[u32; 2],
    characterize_new_points: bool,
    device_queue: &'a (wgpu::Device, wgpu::Queue),
) -> crate::error::Result<GpuState<'a>> {
    ...
    shaders.extend([
        (
            "preprocess_rows",
            (
                include_str!("shaders/preprocess_rows.wgsl"),
                Some(Rc::clone(&direct_dispatcher2d)),
                workgroup_size2d,
            ),
        ),
        (
            "preprocess_cols",
            (
                include_str!("shaders/preprocess_cols.wgsl"),
                Some(Rc::clone(&direct_dispatcher2d)),
                workgroup_size2d,
            ),
        ),
        (
            ...
            (
                ...
                Some(Rc::clone(&indirect_dispatcher)),
                workgroup_size1d,
            ),
        ),
    ]);
    ...
    if tracking_params.characterize | characterize_new_points {
        shaders.extend([
            (
                "characterize",
                (
                    include_str!("shaders/characterize.wgsl"),
                    Some(Rc::clone(&indirect_dispatcher_result)),
                    workgroup_size1d,
                ),
            ),
    ...
    if tracking_params.snr.is_some() | tracking_params.minmass_snr.is_some() {
        shaders.extend([
            (
                "sum_image_outplace",
                (
                    include_str!("shaders/sum_image_outplace.wgsl"),
                    None,
                    workgroup_size1d,
                ),
            ),
            (
                "std_image",
                (
                    include_str!("shaders/std_image.wgsl"),
                    None,
                    workgroup_size1d,
                ),
            ),
            (
                "last_reduction",
                (include_str!("shaders/last_reduction.wgsl"), None, [1, 1, 1]),
            ),
            (
                "sum_image_inplace",
                (
                    include_str!("shaders/sum_image_inplace.wgsl"),
                    None,
                    workgroup_size1d,
                ),
            ),
    ...
    if tracking_params.illumination_sigma.is_some() {
        shaders.extend([
            (
                "sum_frames",
                (
                    include_str!("shaders/correct_illumination/sum_frames.wgsl"),
                    Some(Rc::clone(&direct_dispatcher1d)),
                    workgroup_size1d,
                ),
            ),
            ...
            (
                "smooth_mean_frame",
                (
                    include_str!("shaders/correct_illumination/smooth_mean_frame.wgsl"),
                    Some(Rc::clone(&direct_dispatcher2d)),
                    workgroup_size2d,
                ),
            ),
            (
                ...
                (
                    include_str!("shaders/correct_illumination/correct_illumination.wgsl"),
                    Some(Rc::clone(&direct_dispatcher1d)),
                    workgroup_size1d,
                ),
            ),
    ...
    if tracking_params.adaptive_background.is_some() {
        shaders.extend([(
            "adaptive_filter",
            (
                include_str!("shaders/adaptive_filter.wgsl"),
                Some(Rc::clone(&indirect_dispatcher2)),
                workgroup_size1d,
            ),
        )]);
    }
        ...
        ParamStyle::Trackpy { .. } => {
            ...
            shaders.extend([
                ...
                (
                    "max_rows",
                    (
                        include_str!("shaders/max_rows.wgsl"),
                        Some(Rc::clone(&direct_dispatcher2d)),
                        workgroup_size2d,
                    ),
                ),
                (
                    "extract_max",
                    (
                        include_str!("shaders/extract_max.wgsl"),
                        Some(Rc::clone(&direct_dispatcher2d)),
                        workgroup_size2d,
                    ),
                ),
            ...
            let common_buffers =
                CommonBuffers::create(&tracking_params, &device, size, dims, &mut pipelines);
            ...
            let order = vec![
                ...
                if !characterize_new_points {
                    Some("max_rows".to_string())
                } else {
                    None
                },
                if !characterize_new_points {
                    Some("extract_max".to_string())
                } else {
                    None
                },
                if !characterize_new_points {
            ...
            let buffers = TrackpyGpuBuffers::create(&device, size);

            let flavor = GpuStateFlavor::Trackpy { order, buffers };
        ...
        ParamStyle::Log {
            ...
            shaders.extend([
                ...
                (
                    "logspace_max",
                    (
                        include_str!("shaders/log_style/logspace_max.wgsl"),
                        Some(Rc::clone(&direct_dispatcher2d)),
                        workgroup_size2d,
                    ),
                ),
                ...
                (
                    "separable_log",
                    (
                        include_str!("shaders/log_style/separable_log.wgsl"),
                        Some(Rc::clone(&direct_dispatcher2d)),
                        workgroup_size2d,
                    ),
                ),
            ...
            let flavor = GpuStateFlavor::Log { buffers, radii };
            (flavor, pipelines, common_buffers)
        }
    };
    ...
    */
    ...
    let (walk_result, walk_atomic, walk_dispatch) = if tracking_params.adaptive_background.is_some()
    {
        ...
        (
            &common_buffers.particles_buffer2,
            &common_buffers.atomic_buffer2,
            &indirect_dispatcher2,
        )
    } else {
        (
            &common_buffers.result_buffer,
            &common_buffers.atomic_filtered_buffer,
            &indirect_dispatcher_result,
        )
    };
            ...
            vec![vec![
                ...
                Some((
                    ...
                    common_buffers
                        .raw_frame
                        .as_ref()
                        .unwrap_or(&common_buffers.frame_buffer),
                )),
    ...
    match flavor {
        GpuStateFlavor::Trackpy { ref buffers, .. } => bind_group_entries.extend([
            (
                "max_rows",
                vec![vec![
                    Some((0, &common_buffers.param_buffer)),
        ...
        GpuStateFlavor::Log { ref buffers, .. } => bind_group_entries.extend([(
            "logspace_max",
            (0..3isize)
                .map(|i| {
                    vec![
                        ...
                        Some((
                            1,
                            &buffers.logspace_buffers[((i - 1).rem_euclid(3)) as usize].0,
                        )),
    ...
    passes
        .get_mut("adaptive_filter")
        .as_mut()
        .map(|v| v[1].dispatcher = Some(indirect_dispatcher));

    let characterize = Cell::new(
        passes
            .remove("characterize")
            .map(|mut v| (v.remove(0), workgroup_size1d)),
    );
