use crate::{error::Error, into_slice::IntoSlice, my_dtype};
...
impl<F: IntoSlice, I: Iterator<Item = Result<F, Error>>> FrameProvider
    for Box<dyn FrameProvider<Frame = F, FrameIter = I>>
{
    type Frame = F;
    type FrameIter = I;
    fn get_frame(&self, frame_idx: usize) -> Result<Self::Frame, Error> {
...
impl MinimalETSParser {
    pub fn new<R: Read + Seek>(reader: &mut R) -> io::Result<Self> {
        ...
        for _ in 0..n_used_chunks {
            reader.seek(SeekFrom::Current(4))?;
            let coord = (0..ndimensions)
                .map(|_| reader.read_u32::<LittleEndian>().unwrap() as usize)
                .collect::<Vec<_>>();
    ...
    pub fn iterate_channel<R: Read + Seek>(
        &self,
        reader: R,
        channel: usize,
    ) -> crate::error::Result<ETSIterator<R>> {
        let read_size = self.dims[0] * self.dims[1] * self.data_type.size();
        let offsets = self
            .offsets
            .get(&channel)
            .ok_or(crate::error::Error::ChannelNotFound)?
            .clone();
        Ok(ETSIterator {
            reader,
            // parser: self.reader,
...
impl<R: Read + Seek> ETSIterator<R> {
    ...
    pub fn seek(&mut self, to: usize) -> crate::error::Result<()> {
        match self.offsets.get(to) {
            ...
            None => Err(Error::FrameOutOfBounds {
                vid_len: self.offsets.len(),
                problem_idx: to,
            }),
        }
    }
