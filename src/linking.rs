...
use kd_tree;
use kd_tree::{KdIndexTree, KdPoint, KdTree};
...
impl<'a> Iterator for FrameSubsetter<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        ...
        let mut output = match self.ty {
            ...
            SubsetterType::Agnostic => {
                SubsetterOutput::Agnostic(Array::zeros((0, self.array.shape()[1])))
            }
        };
        ...
        match self.frame_col {
                ...
                loop {
                    ...
                    match output {
                        ...
                        SubsetterOutput::Agnostic(ref mut array) => {
                            array
                                .append(Axis(0), self.array.slice(s![self.idx..self.idx + 1, ..]))
                                .unwrap();
                        }
                    }
                    self.idx += 1;
            ...
            None => {
                match output {
                    ...
                    SubsetterOutput::Characterization(ref mut vec) => match self.r_col {
                        Some(r_col) => {
                            for row in self.array.rows() {
                                let r_row = ResultRow {
                    ...
                    SubsetterOutput::Agnostic(ref mut array) => *array = self.array.into_owned(),
                }
...
impl<'a, T: KdPoint, N: typenum::marker_traits::Unsigned> ReturnDistance2<T, N, usize>
    for KdIndexTree<'a, T>
{
    fn within_radius_rd2(
        &self,
...
impl Linker {
    ...
    pub fn connect<T: KdPoint<Scalar = float, Dim = U2> + std::fmt::Debug>(
        &mut self,
        frame1: &[T],
        frame2: &[T],
    ) -> Result<(Vec<usize>, Vec<usize>)> {
        let prev: Vec<_> = frame1
            .iter()
            .map(|ele| {
    ...
    pub fn advance<T: KdPoint<Scalar = float, Dim = U2> + std::fmt::Debug>(
        ...
        for _ in 0..(self.part_idx - prev_part_idx) {
            self.starting_frame
                .push(DurationBookkeep::new(self.frame_idx));
        }

        self.prev = frame
            .iter()
            .zip(result.iter())
            .map(|(a, b)| ([a.at(0), a.at(1)], *b))
            .collect::<Vec<_>>();
            ...
            for (i, vec) in memvec.iter().enumerate() {
                for entry in vec {
                    ...
                    start_frame.duration =
                        (self.frame_idx - start_frame.start) - (memvec.len() - i);
                }
            }
        }
...
impl LinkingWarning {
    ...
    pub fn handle_warning(&mut self, warning: LinkingWarning, warn_subnet_fraction: f32) {
        ...
        self.check(
            &warning,
            LinkingWarning::SUBNET_FRACTION_TOO_HIGH,
            move || {
                println!(
                    "WARNING: The fraction of linking errors is potentially greater than {}.\n\
Try setting a smaller search range or track with settings that give fewer detections.",
                    warn_subnet_fraction
                );
            },
        );
...
pub fn link<T: KdPoint<Scalar = float, Dim = U2> + std::fmt::Debug>(
    ...
    let dest_points_near_source = src
        .iter()
        .map(|point| tree.within_radius_rd2(point, radius));
    ...
    let max_fraction = std::cmp::max_by(src_fraction, dest_fraction, |a, b| {
    ...
    fn recurse2(
        ...
        let dest_to_src_with_null = dest_to_src[current_dest]
            .iter()
            .map(|ele| (Some(sources[ele.0].1), ele.1))
            .chain(std::iter::once((None, default_score)));
    ...
    let unused_sources = match memory_start_idx {
        Some(idx) => src
            .iter()
            .take(idx)
            .filter(|ele| !used_sources.contains(&ele.1))
            .map(|ele| *ele)
            .collect::<Vec<_>>(),
        None => src
            .iter()
...
pub fn linker_all(
    frame_iter: impl Iterator<Item = crate::error::Result<(Option<usize>, Vec<[float; 2]>)>>,
    radius: float,
    memory: usize,
...
impl LRContainer {
    ...
    fn finish(
        self,
        other: (Option<usize>, Vec<[float; 2]>),
    ) -> (
        (Option<usize>, Vec<[float; 2]>),
        (Option<usize>, Vec<[float; 2]>),
    ) {
        match self.side {
            LR::Left => (self.into_inner(), other),
            LR::Right => (other, self.into_inner()),
        }
    }
...
> {
    let next_item = match to_hit.side {
        LR::Left => frame_iter2.next(),
        LR::Right => frame_iter1.next(),
    };
    ...
    match to_hit.cmp(&next_item) {
        ...
        std::cmp::Ordering::Equal => return Ok(Some(to_hit.finish(next_item))),
        std::cmp::Ordering::Less => {
            to_hit.fill_other(&next_item, results1, results2, linker);
            to_hit.fill(results1, results2, linker);
            return sync_iterators(
                frame_iter1,
                frame_iter2,
                to_hit.other(next_item),
                linker,
                results1,
                results2,
            );
        ...
        match (frame1, frame2) {
            (Some(Ok(frame1)), Some(Ok(frame2))) => {
                let both_frames = match frame1.0.unwrap().cmp(&frame2.0.unwrap()) {
                    std::cmp::Ordering::Equal => Some((frame1, frame2)),
                    std::cmp::Ordering::Greater => {
                        let to_hit = LRContainer {
                            item: frame1,
                            side: LR::Left,
                        };
            ...
            (Some(Ok(frame1)), None) => fill_base(&frame1, &mut results1, &mut linker),
            (None, Some(Ok(frame2))) => fill_base(&frame2, &mut results2, &mut linker),
            (None, None) => break,
